...
interface PlotAreaType {
    reset(): void
}
const typedModules = modules as ModulesType
const typedPlotArea = plotArea as unknown as PlotAreaType
const typedOppositeDirection: Record<string, string> =
    oppositeDirection as Record<string, string>

/**
 * Backward compatibility - needs to be deprecated
 * @param {string} obj - the object to be rectified
 * @category data
 */
...
function loadModule(data: ModuleData, scope: Scope): void {
    ...
    var obj = new typedModules[rectifyObjectType(data.objectType)](
        data.x,
        data.y,
        scope,
    ...
    obj.labelDirection =
        data.labelDirection ||
        typedOppositeDirection[fixDirection[obj.direction]]
...
export function loadScope(scope: Scope, data: ScopeData): void {
    ...
    for (let i = 0; i < data.allNodes.length; i++) {
        constructNodeConnections(scope.allNodes[i], data.allNodes[i] as any)
    }
    // Load all modules
    ...
    if (data.layout) {
        ...
        scope.layout = {
            width: 100,
            height: Math.max(scope.Input.length, scope.Output.length) * 20 + 20,
            title_x: 50,
            title_y: 13,
        }
...
export default function load(data?: ProjectData): void {
    ...
    for (let i = 0; i < data.scopes.length; i++) {
        ...
        if (data.scopes[i].verilogMetadata) {
            isVerilogCircuit =
                data.scopes[i].verilogMetadata?.isVerilogCircuit || false
            isMainCircuit =
                data.scopes[i].verilogMetadata?.isMainCircuit || false
        }
        // Create new circuit
        ...
        if (scope) {
            // Load circuit data
            loadScope(scope as unknown as Scope, data.scopes[i])

            // Focus circuit
            globalScope = scope as unknown as Scope
    ...
    if (data.orderedTabs && data.orderedTabs.length > 0) {
        ...
        circuit_list.value.sort((a, b) => {
            return (
                (data.orderedTabs as string[]).indexOf(String(a.id)) -
                (data.orderedTabs as string[]).indexOf(String(b.id))
            )
        })
    }
